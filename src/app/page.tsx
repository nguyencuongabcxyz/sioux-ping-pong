  ...
  const fetchBracketData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch bracket data: ${response.status} - ${errorData}`)
      }
      const data = await response.json()
      console.log('Bracket data received:', data)
      
      // Log specific bracket info
      if (data.bracket) {
        console.log('Quarter finals found:', data.bracket.quarterFinals?.length || 0)
        console.log('Quarter finals data:', data.bracket.quarterFinals)
      } else {
        console.log('No bracket data returned, knockout generated:', data.knockoutGenerated)
      }
    ...
    return (
      <div className="space-y-6 sm:space-y-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-red-800 mb-2">Error Loading Bracket</h3>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 sm:p-6 border">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex-shrink-0 space-x-2">
              ...
              >
                {loading ? 'Loading...' : 'Refresh Bracket'}
              </button>
              
              {/* Debug Info - Show in development or if there are issues */}
              <details className="inline-block">
                <summary className="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm rounded-lg cursor-pointer transition-colors">
                  Debug Info
                </summary>
                <div className="absolute mt-2 p-4 bg-white border border-gray-200 rounded-lg shadow-lg z-10 text-xs">
                  <div className="space-y-1 text-left min-w-64">
                    <div><strong>Group Stage:</strong> {bracketData.groupStageCompleted ? '✅ Complete' : '❌ Incomplete'}</div>
                    <div><strong>Knockout Generated:</strong> {bracketData.knockoutGenerated ? '✅ Yes' : '❌ No'}</div>
                    <div><strong>Bracket Data:</strong> {bracketData.bracket ? '✅ Present' : '❌ None'}</div>
                    {bracketData.bracket && (
                      <div><strong>Quarter Finals:</strong> {bracketData.bracket.quarterFinals?.length || 0} matches</div>
                    )}
                    <div className="pt-2 border-t">
                      <a href="/api/tournament/debug" target="_blank" className="text-blue-600 hover:underline">
                        View Debug API →
                      </a>
                    </div>
                  </div>
